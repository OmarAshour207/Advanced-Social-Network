<?php
	class Message extends User
	{
		...
		{
			$stmt = $this->pdo->prepare("SELECT COUNT(`messageID`) AS `totalMessages`,(SELECT COUNT(`ID`) FROM `notification` WHERE `notificationFor` = :user_id AND `status` = '0') AS `totalN` FROM `messages` WHERE `messageTo` = :user_id AND `status` = '0'");
			$stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
			$stmt->execute();
