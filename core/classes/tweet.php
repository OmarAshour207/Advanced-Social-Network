<?php
	class Tweet extends User{
							...
							'.((!empty($tweet->retweetMsg) && $tweet->tweetID === $retweet['tweetID'] OR $tweet->retweetID > 0) ? 
													...
													<div class="t-s-head-content">
														<div class="t-h-c-name">
															<span><a href="'.BASE_URL.$user->user_name.'">'.$user->screenName.'</a></span>
												...
												<div class="t-s-b-inner">
													<div class="t-s-b-inner-in">
														...
														'.((!empty($tweet->tweetImage)) ? '
															...
															<div>
																<div class="t-h-c-name">
																	<span><a href="'.BASE_URL.$tweet->user_name.'">'.$tweet->screenName.'</a></span>
		...
		{
			$stmt = $this->pdo->prepare("SELECT `user_id`, `user_name`, `screenName`, `profileImage` FROM `users` WHERE `user_name` LIKE :mention OR `screenName` LIKE :mention LIMIT 5");
		...
		public function addTrend($hashtag)
		{
			...
			$sql = "INSERT INTO `trends` (`hashtag`, `createOn`) VALUES(:hashtag, CURRENT_TIMESTAMP)";
		...
		public function addMention($status, $user_id, $tweet_id)
		{
			...
			$sql = "SELECT * FROM `users` WHERE `user_name` = :mention";
			$data = '';
			foreach ($result as $trend) {
				if($stmt = $this->pdo->prepare($sql))
				{
		...
		public function trends()
		{
			...
			echo '<div class="trend-wrapper"><div class="trend-inner">
		...
		{
			$stmt = $this->pdo->prepare("SELECT DISTINCT * FROM `tweets` INNER JOIN `users` ON `tweetBy` = `user_id` WHERE `status` LIKE :hashtag OR `retweetMsg` = :hashtag GROUP BY `user_id`");
			$stmt->bindValue(":hashtag", '%#'.$hashtag.'%', PDO::PARAM_STR);
			$stmt->execute();
			return $stmt->fetchAll(PDO::FETCH_OBJ);
		}
