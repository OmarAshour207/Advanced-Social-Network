<?php
	class User{
		...
		{
			$stmt = $this->pdo->prepare("SELECT `user_id`, `user_name`, `screenName`, `profileImage`, `profileCover` FROM `users` WHERE `user_name` LIKE ? OR `screenName` LIKE ?");
		...
		{
			$stmt = $this->pdo->prepare("SELECT `user_name` FROM `users` WHERE `username` = :username");
		...
		{
			$stmt = $this->pdo->prepare("SELECT `user_id` FROM `users` WHERE `user_name` =:username");
			$stmt->bindParam(":username", $username, PDO::PARAM_STR);
			$stmt->execute();
			...
			if(in_array($ext , $allowed_ext) === true)
			{
				if($error === 0)
				{
					if($fileSize <= 209272152)
					{
						...
						move_uploaded_file($fileTmp, $_SERVER['DOCUMENT_ROOT'].'/Social_Network_Twitter/'.$fileRoot);
						return $fileRoot;
					}
					else
					{
